...
uint32_t init_timer ( uint8_t timer_num, uint32_t TimerInterval )
{
  if ( timer_num == 0 )
  {
	LPC_TIM0->MR0 = TimerInterval/3;	//gameover
...
// <<< end of configuration section >>>    
	NVIC_SetPriority(TIMER0_IRQn, 0);	/* less priority than buttons and timer0*/
	NVIC_EnableIRQ(TIMER0_IRQn);
	return (1);
  }
  else if ( timer_num == 1 )
  {
	LPC_TIM1->MR1 = TimerInterval*0.5;
	LPC_TIM1->MR0 = TimerInterval*0.4;
	LPC_TIM1->MR2 = 0xFFFFFFFF;
	LPC_TIM1->MCR = 201;				/* Interrupt and Reset on MR1 */
